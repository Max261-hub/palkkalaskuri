...
DB_NAME = "kevytyrittaja_v2.db"

def luo_tietokanta():
    with sqlite3.connect(DB_NAME) as conn:
        c = conn.cursor()
        # Käyttäjätaulu: salasana tallennetaan hashattuna tietoturvan vuoksi
        ...
        # Palkkataulu: sisältää kaikki laskentakentät historian seurantaa varten
        c.execute('''CREATE TABLE IF NOT EXISTS palkat 
                     (id INTEGER PRIMARY KEY AUTOINCREMENT, kayttaja_id INTEGER,
                      ...
                      tyel REAL, vero_euro REAL, netto REAL, pvm TEXT)''')
        conn.commit()
...
def hae_historia(k_id):
    with sqlite3.connect(DB_NAME) as conn:
        # Hakee historian laskevassa järjestyksessä (uusimmat ensin)
